...
## 8. 주식 알림 기능 추가 (2026-06-20)
- **결정**: 프론트=Vanilla JS, 데이터=더미+연동가이드, 백엔드=Express + Netlify Functions 어댑터 둘 다.
- **위치**: `stock-alert/`(프론트+Express 서버), `netlify/functions/stocks-alert.js`(서버리스 신청 API).
- **프론트**(`stock-alert/index.html`+`app.js`): 모바일퍼스트 핀테크 톤(토스/카카오페이증권), 종목요약+AI 3줄, 차트(Chart.js+핀치줌), 즐겨찾기 토글, 바텀시트 알림폼. 핵심: 알림종류 드롭다운의 '고점 대비 % 하락' 선택 시 하락률 입력+칩 동적 노출. 전화번호 입력. `window.STOCK_API_BASE`로 백엔드 주소 주입.
- **백엔드**(`stock-alert/server/`): `POST /api/stocks/alert`(검증+저장), 시세조회, 점검 스케줄러(node-cron). `services/kakao.js`의 `checkAndSendKakaoAlert`가 고점대비 하락률 판정→알림톡 발송(같은날 1회). DB는 파일기반(`services/db.js`, 운영시 Postgres/Mongo 교체), 시세는 더미(`marketData.js`, KIS/Finnhub 교체지점 주석).
- **검증**: 전 파일 node --check 통과, 검증 로직(201/에러), 하락 트리거(15%↔20%=발송, 25%↔20%=미발송) 확인.
- **연동**: `stock-alert/README_연동.md`에 마운트 팁, .env, 스케줄 함수, 엔드포인트 표.
- ⚠️ 서버리스 신청 API는 `@netlify/blobs` 사용 → 배포 시 사이트 빌드와 함께 동작(이미 의존성 transitive 확보).
