/* article.js — 정적 글 페이지 인터랙션 (목차/진행률/글자크기/공유/조회수) */
(function(){
  "use strict";
  ...
  /* 조회수 (Abacus 무료 카운터 — 가입 불필요) */
  var metaBox=document.querySelector(".a-meta");
  if(metaBox){
    var vEl=document.createElement("span");
    vEl.className="a-views";
    metaBox.appendChild(vEl);
    var key=(location.pathname.split("/").pop()||"home").replace(/\.html$/,"").replace(/[^a-zA-Z0-9_-]/g,"-").replace(/^-+|-+$/g,"")||"home";
    fetch("https://abacus.jasoncameron.dev/hit/jisikchango/"+key)
      .then(function(r){return r.json();})
      .then(function(d){ if(d&&typeof d.value==="number"){ vEl.textContent="👁 "+d.value.toLocaleString()+" 조회"; } })
      .catch(function(){});
  }
})();
